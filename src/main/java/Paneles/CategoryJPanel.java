...
public class CategoryJPanel extends JPanel {
    ...
    private JPanel createHeaderPanel() {
        ...
        JLabel lblSubtitle = new JLabel("CRUD de categorías.");
    ...
    private void loadTable() {
        try {
            ...
            List<Category> list = controller.listActive(currentSearch, currentPage, limit);

            for (Category category : list) {
                tableModel.addRow(new Object[]{
                    category.getIdCategory(),
                    category.getName(),
                    shortText(category.getDescription()),
                    ""
                });
            }
    ...
    private void openFormDialog(Category category, boolean readOnly) {
        boolean isEdit = category != null && !readOnly;

        JDialog dialog = new JDialog((JFrame) SwingUtilities.getWindowAncestor(this), true);
        dialog.setTitle(readOnly ? "Detalle de Categoría" : isEdit ? "Editar Categoría" : "Nueva Categoría");
        ...
        if (readOnly && category != null) {
            JTextField txtId = createFormTextField();
            txtId.setText(String.valueOf(category.getIdCategory()));
        ...
        if (category != null) {
            txtName.setText(category.getName() == null ? "" : category.getName());
        ...
        if (readOnly && category != null) {
            ...
            txtStatus.setText(getStatusText(category.getStatus()));
            txtStatus.setEditable(false);

            JTextField txtCreatedAt = createFormTextField();
            txtCreatedAt.setText(formatDate(category.getCreatedAt()));
            txtCreatedAt.setEditable(false);

            JTextField txtUpdatedAt = createFormTextField();
            txtUpdatedAt.setText(formatDate(category.getUpdatedAt()));
        ...
        if (!readOnly) {
            ...
            btnSave.addActionListener(e -> {
                try {
                    if (isEdit) {
                        controller.update(category.getIdCategory(), txtName.getText(), txtDescription.getText());
                    } else {
                    ...
                    JOptionPane.showMessageDialog(
                            this,
                            isEdit ? "Categoría actualizada correctamente." : "Categoría registrada correctamente.",
                            "Operación exitosa",
    ...
    private void viewDetail(int idCategory) {
        try {
            Category category = controller.findById(idCategory);
            openFormDialog(category, true);
        } catch (Exception e) {
            showError(e.getMessage());
        }
    }

    private void editCategory(int idCategory) {
        try {
            Category category = controller.findById(idCategory);
            openFormDialog(category, false);
        } catch (Exception e) {
            showError(e.getMessage());
        }
    }

    private void deleteLogical(int idCategory) {
        ...
        try {
            controller.deleteLogical(idCategory);
            loadTable();
    ...
    private void openInactiveDialog() {
        ...
        inactiveDialog.setTitle("Categorías Inactivas");
        ...
        JLabel lblTitle = new JLabel("Categorías Inactivas");
        lblTitle.setFont(new Font("Segoe UI", Font.BOLD, 24));
        lblTitle.setForeground(textColor);
    ...
    private void loadInactiveTable() {
        try {
            ...
            List<Category> list = controller.listInactive(inactiveSearch, inactivePage, inactiveLimit);
            ...
            for (Category category : list) {
                inactiveTableModel.addRow(new Object[]{
                    ...
                    category.getName(),
                    shortText(category.getDescription()),
                    category.getDeletedAt() == null ? "-" : formatDate(category.getDeletedAt()),
                    ""
                });
            }
    ...
    private void restoreCategory(int idCategory) {
        int option = JOptionPane.showConfirmDialog(
                this,
                "¿Deseas restaurar esta categoría?",
    ...
    private void deletePhysical(int idCategory) {
        int option = JOptionPane.showConfirmDialog(
                this,
                "¿Deseas eliminar físicamente esta categoría?\nEsta acción no se puede deshacer.",
        ...
        try {
            controller.deletePhysical(idCategory);
            loadInactiveTable();
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            frame.setContentPane(new CategoryJPanel());
