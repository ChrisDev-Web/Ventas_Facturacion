...
public class DocumentTypeJPanel extends JPanel {

    private final DocumentTypeController controller;
    ...
    public DocumentTypeJPanel() {
        this.controller = new DocumentTypeController();
        initUI();
        loadTable();
    }
    ...
    private JPanel createHeaderPanel() {
        ...
        JLabel lblTitle = new JLabel("Tipo de Documentos");
        lblTitle.setFont(new Font("Segoe UI", Font.BOLD, 30));
        lblTitle.setForeground(textColor);

        JLabel lblSubtitle = new JLabel("Administra los tipos de documentos del sistema.");
    ...
    private void loadTable() {
        try {
            ...
            List<DocumentType> list = controller.listActive(currentSearch, currentPage, limit);

            for (DocumentType documentType : list) {
                tableModel.addRow(new Object[]{
                    documentType.getIdDocumentType(),
                    documentType.getName(),
                    shortText(documentType.getDescription()),
                    ""
                });
            }
    ...
    private void openFormDialog(DocumentType documentType, boolean readOnly) {
        boolean isEdit = documentType != null && !readOnly;

        JDialog dialog = new JDialog((JFrame) SwingUtilities.getWindowAncestor(this), true);
        dialog.setTitle(readOnly ? "Detalle del Tipo de Documento" : isEdit ? "Editar Tipo de Documento" : "Nuevo Tipo de Documento");
        ...
        if (readOnly && documentType != null) {
            JLabel lblId = createFormLabel("ID");

            JTextField txtId = createFormTextField();
            txtId.setText(String.valueOf(documentType.getIdDocumentType()));
        ...
        if (documentType != null) {
            txtName.setText(documentType.getName() == null ? "" : documentType.getName());
            txtDescription.setText(documentType.getDescription() == null ? "" : documentType.getDescription());
        }
        ...
        if (readOnly && documentType != null) {
            ...
            txtStatus.setText(getStatusText(documentType.getStatus()));
            txtStatus.setEditable(false);

            JLabel lblCreatedAt = createFormLabel("Fecha de creación");

            JTextField txtCreatedAt = createFormTextField();
            txtCreatedAt.setText(formatDate(documentType.getCreatedAt()));
            ...
            txtUpdatedAt.setText(formatDate(documentType.getUpdatedAt()));
        ...
        if (!readOnly) {
            ...
            btnSave.addActionListener(e -> {
                try {
                    if (isEdit) {
                        controller.update(documentType.getIdDocumentType(), txtName.getText(), txtDescription.getText());
                    } else {
                    ...
                    JOptionPane.showMessageDialog(
                            this,
                            isEdit ? "Tipo de documento actualizado correctamente." : "Tipo de documento registrado correctamente.",
                            "Operación exitosa",
    ...
    private void viewDetail(int idDocumentType) {
        try {
            DocumentType documentType = controller.findById(idDocumentType);
            openFormDialog(documentType, true);
        } catch (Exception e) {
            showError(e.getMessage());
        }
    }

    private void editDocumentType(int idDocumentType) {
        try {
            DocumentType documentType = controller.findById(idDocumentType);
            openFormDialog(documentType, false);
        } catch (Exception e) {
            showError(e.getMessage());
        }
    }

    private void deleteLogical(int idDocumentType) {
        int option = JOptionPane.showConfirmDialog(
                this,
                "¿Deseas eliminar este tipo de documento?\nSe eliminará de forma lógica.",
        ...
        try {
            controller.deleteLogical(idDocumentType);
            loadTable();
    ...
    private void openInactiveDialog() {
        ...
        inactiveDialog.setTitle("Tipos de Documentos Inactivos");
        ...
        JLabel lblTitle = new JLabel("Tipos de Documentos Inactivos");
    ...
    private void loadInactiveTable() {
        try {
            ...
            List<DocumentType> list = controller.listInactive(inactiveSearch, inactivePage, inactiveLimit);
            ...
            for (DocumentType documentType : list) {
                inactiveTableModel.addRow(new Object[]{
                    ...
                    documentType.getName(),
                    shortText(documentType.getDescription()),
                    documentType.getDeletedAt() == null ? "-" : documentType.getDeletedAt().format(dateFormatter),
                    ""
                });
            }
    ...
    private void restoreDocumentType(int idDocumentType) {
        int option = JOptionPane.showConfirmDialog(
                this,
                "¿Deseas restaurar este tipo de documento?",
        ...
        try {
            controller.restore(idDocumentType);
            loadInactiveTable();
            loadTable();

            JOptionPane.showMessageDialog(
                    this,
                    "Tipo de documento restaurado correctamente.",
                    "Operación exitosa",
                    JOptionPane.INFORMATION_MESSAGE
            );
    ...
    private void deletePhysical(int idDocumentType) {
        int option = JOptionPane.showConfirmDialog(
                this,
                "¿Deseas eliminar físicamente este tipo de documento?\nEsta acción no se puede deshacer.",
        ...
        try {
            controller.deletePhysical(idDocumentType);
            loadInactiveTable();

            JOptionPane.showMessageDialog(
                    this,
                    "Tipo de documento eliminado físicamente.",
                    "Operación exitosa",
                    JOptionPane.INFORMATION_MESSAGE
            );
    ...
    private class InactiveActionsEditor extends AbstractCellEditor implements TableCellEditor {
        ...
        public InactiveActionsEditor(JTable table) {
            ...
            btnRestore.addActionListener(e -> {
                fireEditingStopped();
                restoreDocumentType(selectedId);
            });
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            JFrame frame = new JFrame("Tipos de Documentos");
            frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
            frame.setSize(1100, 700);
            frame.setLocationRelativeTo(null);
            frame.setContentPane(new DocumentTypeJPanel());
