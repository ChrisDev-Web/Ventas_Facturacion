...
public class ClientJPanel extends JPanel {
    ...
    private JPanel createHeaderPanel() {
        ...
        JLabel lblSubtitle = new JLabel("Administra los clientes del sistema.");
    ...
    private JPanel createTablePanel() {
        ...
        tableModel = new DefaultTableModel(
                new Object[]{"ID", "Cliente", "Documento", "Correo", "Acciones"},
                0
        ) {
            @Override
    ...
    private void loadTable() {
        try {
            ...
            List<Client> list = controller.listActive(currentSearch, currentPage, limit);
            ...
            for (Client client : list) {
                tableModel.addRow(new Object[]{
                    ...
                    nullToDash(client.getEmail()),
                    ""
                });
            }

            lblPagination.setText("PÃ¡gina " + currentPage + " de " + totalPages + " | Total: " + totalRecords);
    ...
    private void openFormDialog(Client client, boolean readOnly) {
        boolean isEdit = client != null && !readOnly;

        JDialog dialog = new JDialog((JFrame) SwingUtilities.getWindowAncestor(this), true);
        dialog.setTitle(readOnly ? "Detalle del Cliente" : isEdit ? "Editar Cliente" : "Nuevo Cliente");
        dialog.setSize(readOnly ? 760 : 780, readOnly ? 720 : 680);
        ...
        btnSearchDni.addActionListener(e -> searchClientByDni(
                cmbDocumentType,
                txtDocumentNumber,
                txtName,
                txtLastNamePaternal,
        ...
        if (client != null) {
            ...
            txtName.setText(nullToEmpty(client.getName()));
            txtLastNamePaternal.setText(nullToEmpty(client.getLastNamePaternal()));
            txtLastNameMaternal.setText(nullToEmpty(client.getLastNameMaternal()));

            txtDocumentNumber.setText(nullToEmpty(client.getDocumentNumber()));
            txtPhone.setText(nullToEmpty(client.getPhone()));
            txtEmail.setText(nullToEmpty(client.getEmail()));
            txtAddress.setText(nullToEmpty(client.getAddress()));

            selectComboItem(cmbDocumentType, client.getIdDocumentType());
            selectComboItem(cmbRegion, client.getIdRegions());

            if (client.getIdRegions() != null) {
                loadCombo(cmbProvince, controllerListProvinces(client.getIdRegions()), "Seleccione");
                selectComboItem(cmbProvince, client.getIdProvinces());
            }

            if (client.getIdProvinces() != null) {
                loadCombo(cmbDistrict, controllerListDistricts(client.getIdProvinces()), "Seleccione");
                selectComboItem(cmbDistrict, client.getIdDistrict());
            }

            txtStatus.setText(getStatusText(client.getStatus()));
            txtCreatedAt.setText(formatDate(client.getCreatedAt()));
            txtUpdatedAt.setText(formatDate(client.getUpdatedAt()));
        } else {
        ...
        row = addField(form, gbc, row, "Apellido materno", txtLastNameMaternal, "Tipo de documento", cmbDocumentType);
        row = addField(
                form,
                gbc,
                row,
        ...
        row = addField(form, gbc, row, "Correo", txtEmail, "Region", cmbRegion);
        row = addField(form, gbc, row, "Provincia", cmbProvince, "Distrito", cmbDistrict);
        row = addField(form, gbc, row, "Direccion", addressScroll, "", new JLabel(""));

        if (readOnly && client != null) {
            row = addField(form, gbc, row, "Fecha de creaciÃ³n", txtCreatedAt, "Fecha de actualizaciÃ³n", txtUpdatedAt);
        }
        ...
        if (!readOnly) {
            ...
            btnSave.addActionListener(e -> {
                try {
                    ...
                    if (isEdit) {
                        formClient.setIdClient(client.getIdClient());
                    }

                    formClient.setName(txtName.getText());
                    formClient.setLastNamePaternal(txtLastNamePaternal.getText());
                    formClient.setLastNameMaternal(txtLastNameMaternal.getText());

                    formClient.setIdDocumentType(getSelectedNullableId(cmbDocumentType));
                    formClient.setDocumentNumber(txtDocumentNumber.getText());

                    formClient.setPhone(txtPhone.getText());
                    formClient.setEmail(txtEmail.getText());
                    formClient.setAddress(txtAddress.getText());

                    formClient.setIdRegions(getSelectedNullableId(cmbRegion));
                    formClient.setIdProvinces(getSelectedNullableId(cmbProvince));
                    formClient.setIdDistrict(getSelectedNullableId(cmbDistrict));

                    if (isEdit) {
                        controller.update(formClient);
                    } else {
                        controller.create(formClient);
                    }
                    ...
                    JOptionPane.showMessageDialog(
                            this,
                            isEdit ? "Cliente actualizado correctamente." : "Cliente registrado correctamente.",
                            "OperaciÃ³n exitosa",
    ...
    private void searchClientByDni(
        ...
        try {
            ...
            Client foundClient = controller.searchByDni(txtDocumentNumber.getText());

            txtName.setText(nullToEmpty(foundClient.getName()));
            txtLastNamePaternal.setText(nullToEmpty(foundClient.getLastNamePaternal()));
            txtLastNameMaternal.setText(nullToEmpty(foundClient.getLastNameMaternal()));
            txtDocumentNumber.setText(nullToEmpty(foundClient.getDocumentNumber()));
    ...
    private void viewDetail(int idClient) {
        try {
            Client client = controller.findById(idClient);
            openFormDialog(client, true);
        } catch (Exception e) {
            showError(e.getMessage());
        }
    }
    ...
    private void editClient(int idClient) {
        try {
            Client client = controller.findById(idClient);
            openFormDialog(client, false);
        } catch (Exception e) {
            showError(e.getMessage());
        }
    }

    private void deleteLogical(int idClient) {
        int option = JOptionPane.showConfirmDialog(
                this,
                "Â¿Deseas eliminar este cliente?\nSe eliminarÃ¡ de forma lÃ³gica.",
        ...
        try {
            controller.deleteLogical(idClient);
            loadTable();
    ...
    private void openInactiveDialog() {
        ...
        inactiveDialog.setTitle("Clientes Inactivos");
        ...
        JLabel lblTitle = new JLabel("Clientes Inactivos");
    ...
    private JPanel createInactiveTablePanel() {
        ...
        inactiveTableModel = new DefaultTableModel(
                new Object[]{"ID", "Cliente", "Correo", "Eliminado", "Acciones"},
                0
        ) {
            @Override
            public boolean isCellEditable(int row, int column) {
    ...
    private void loadInactiveTable() {
        try {
            ...
            List<Client> list = controller.listInactive(inactiveSearch, inactivePage, inactiveLimit);
            ...
            for (Client client : list) {
                inactiveTableModel.addRow(new Object[]{
                    ...
                    client.getFullName(),
                    nullToDash(client.getEmail()),
                    client.getDeletedAt() == null ? "-" : formatDate(client.getDeletedAt()),
                    ""
                });
            }

            lblInactivePagination.setText("PÃ¡gina " + inactivePage + " de " + inactiveTotalPages + " | Total: " + totalRecords);
    ...
    private void restoreClient(int idClient) {
        int option = JOptionPane.showConfirmDialog(
                this,
                "Â¿Deseas restaurar este cliente?",
    ...
    private void deletePhysical(int idClient) {
        int option = JOptionPane.showConfirmDialog(
                this,
                "Â¿Deseas eliminar fÃ­sicamente este cliente?\nEsta acciÃ³n no se puede deshacer.",
                "Confirmar eliminaciÃ³n fÃ­sica",
                JOptionPane.YES_NO_OPTION,
        ...
        try {
            controller.deletePhysical(idClient);
            loadInactiveTable();

            JOptionPane.showMessageDialog(
                    this,
                    "Cliente eliminado fÃ­sicamente.",
                    "OperaciÃ³n exitosa",
                    JOptionPane.INFORMATION_MESSAGE
            );
    ...
    private String buildDocument(Client client) {
        String type = client.getDocumentTypeName() == null ? "" : client.getDocumentTypeName();
        String number = client.getDocumentNumber() == null ? "" : client.getDocumentNumber();
    ...
    private class InactiveActionsRenderer extends JPanel implements TableCellRenderer {
        ...
        public InactiveActionsRenderer() {
            ...
            add(createActionButton(FontAwesome.TRASH, dangerColor, "Eliminar fÃ­sico"));
        }
    ...
    private class InactiveActionsEditor extends AbstractCellEditor implements TableCellEditor {
        ...
        public InactiveActionsEditor(JTable table) {
            ...
            JButton btnDelete = createActionButton(FontAwesome.TRASH, dangerColor, "Eliminar fÃ­sico");

            btnRestore.addActionListener(e -> {
                fireEditingStopped();
                restoreClient(selectedId);
            });
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            frame.setContentPane(new ClientJPanel());
