...
public class PaymentMethodJPanel extends JPanel {

    private final PaymentMethodController controller;
    ...
    private JPanel createHeaderPanel() {
        ...
        JLabel lblSubtitle = new JLabel("CRUD de métodos de pago.");
    ...
    private void loadTable() {
        try {
            ...
            List<PaymentMethod> list = controller.listActive(currentSearch, currentPage, limit);

            for (PaymentMethod paymentMethod : list) {
                tableModel.addRow(new Object[]{
                    paymentMethod.getIdPaymentMethod(),
                    paymentMethod.getName(),
                    shortText(paymentMethod.getDescription()),
                    ""
                });
            }
    ...
    private void openFormDialog(PaymentMethod paymentMethod, boolean readOnly) {
        boolean isEdit = paymentMethod != null && !readOnly;

        JDialog dialog = new JDialog((JFrame) SwingUtilities.getWindowAncestor(this), true);
        dialog.setTitle(readOnly ? "Detalle del Método de Pago" : isEdit ? "Editar Método de Pago" : "Nuevo Método de Pago");
        ...
        if (readOnly && paymentMethod != null) {
            JTextField txtId = createFormTextField();
            txtId.setText(String.valueOf(paymentMethod.getIdPaymentMethod()));
        ...
        if (paymentMethod != null) {
            txtName.setText(paymentMethod.getName() == null ? "" : paymentMethod.getName());
            txtDescription.setText(paymentMethod.getDescription() == null ? "" : paymentMethod.getDescription());
        }
        ...
        if (readOnly && paymentMethod != null) {
            ...
            txtStatus.setText(getStatusText(paymentMethod.getStatus()));
            txtStatus.setEditable(false);

            JTextField txtCreatedAt = createFormTextField();
            txtCreatedAt.setText(formatDate(paymentMethod.getCreatedAt()));
            txtCreatedAt.setEditable(false);

            JTextField txtUpdatedAt = createFormTextField();
            txtUpdatedAt.setText(formatDate(paymentMethod.getUpdatedAt()));
        ...
        if (!readOnly) {
            ...
            btnSave.addActionListener(e -> {
                try {
                    if (isEdit) {
                        controller.update(paymentMethod.getIdPaymentMethod(), txtName.getText(), txtDescription.getText());
                    } else {
                    ...
                    JOptionPane.showMessageDialog(
                            this,
                            isEdit ? "Método de pago actualizado correctamente." : "Método de pago registrado correctamente.",
                            "Operación exitosa",
    ...
    private void viewDetail(int idPaymentMethod) {
        try {
            PaymentMethod paymentMethod = controller.findById(idPaymentMethod);
            openFormDialog(paymentMethod, true);
        } catch (Exception e) {
            showError(e.getMessage());
        }
    }

    private void editPaymentMethod(int idPaymentMethod) {
        try {
            PaymentMethod paymentMethod = controller.findById(idPaymentMethod);
            openFormDialog(paymentMethod, false);
        } catch (Exception e) {
            showError(e.getMessage());
        }
    }

    private void deleteLogical(int idPaymentMethod) {
        ...
        try {
            controller.deleteLogical(idPaymentMethod);
            loadTable();
    ...
    private void openInactiveDialog() {
        ...
        inactiveDialog.setTitle("Métodos de Pago Inactivos");
        ...
        JLabel lblTitle = new JLabel("Métodos de Pago Inactivos");
        lblTitle.setFont(new Font("Segoe UI", Font.BOLD, 24));
        lblTitle.setForeground(textColor);
    ...
    private void loadInactiveTable() {
        try {
            ...
            List<PaymentMethod> list = controller.listInactive(inactiveSearch, inactivePage, inactiveLimit);
            ...
            for (PaymentMethod paymentMethod : list) {
                inactiveTableModel.addRow(new Object[]{
                    ...
                    paymentMethod.getName(),
                    shortText(paymentMethod.getDescription()),
                    paymentMethod.getDeletedAt() == null ? "-" : formatDate(paymentMethod.getDeletedAt()),
                    ""
                });
            }
    ...
    private void restorePaymentMethod(int idPaymentMethod) {
        int option = JOptionPane.showConfirmDialog(
                this,
                "¿Deseas restaurar este método de pago?",
        ...
        try {
            controller.restore(idPaymentMethod);
            loadInactiveTable();
            loadTable();

            JOptionPane.showMessageDialog(
                    this,
                    "Método de pago restaurado correctamente.",
                    "Operación exitosa",
                    JOptionPane.INFORMATION_MESSAGE
            );
    ...
    private void deletePhysical(int idPaymentMethod) {
        int option = JOptionPane.showConfirmDialog(
                this,
                "¿Deseas eliminar físicamente este método de pago?\nEsta acción no se puede deshacer.",
        ...
        try {
            controller.deletePhysical(idPaymentMethod);
            loadInactiveTable();

            JOptionPane.showMessageDialog(
                    this,
                    "Método de pago eliminado físicamente.",
                    "Operación exitosa",
                    JOptionPane.INFORMATION_MESSAGE
            );
    ...
    private class InactiveActionsEditor extends AbstractCellEditor implements TableCellEditor {
        ...
        public InactiveActionsEditor(JTable table) {
            ...
            btnRestore.addActionListener(e -> {
                fireEditingStopped();
                restorePaymentMethod(selectedId);
            });
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            JFrame frame = new JFrame("Métodos de Pago");
            frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
            frame.setSize(1000, 680);
            frame.setLocationRelativeTo(null);
            frame.setContentPane(new PaymentMethodJPanel());
